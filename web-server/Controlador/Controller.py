...
class Controller(object):
    ...
    def get_publicaciones_perfil(self,email):
        ...
        rows = Comandos.consulta('SELECT id,id_materia,fecha,id_archivo,id_usuario,contenido,hora FROM publicaciones WHERE id_usuario = \'%s\' ORDER BY fecha DESC;' % (id_usuario[0][0]))
        cadena = ""
        if rows == []:
